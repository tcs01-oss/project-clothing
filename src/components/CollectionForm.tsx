...
export const getInitialCollectionState = (
    ...
    if (releaseType === "footwear") {
      return {
        basicDetails: {
          ...
          topSizes: ["6", "7", "8", "9", "10", "11", "12"],
          bottomSizes: ["6", "7", "8", "9", "10", "11", "12"],
          sizeGuideImage: "",
  ...
  return {
    basicDetails: {
      ...
      sizes: product.sizes || (isFootwear ? ["6", "7", "8", "9", "10", "11", "12"] : ["S", "M", "L", "XL", "XXL"]),
      topSizes: product.topSizes || product.sizes || (isFootwear ? ["6", "7", "8", "9", "10", "11", "12"] : ["S", "M", "L", "XL", "XXL"]),
      bottomSizes: product.bottomSizes || product.sizes || (isFootwear ? ["6", "7", "8", "9", "10", "11", "12"] : ["S", "M", "L", "XL", "XXL"]),
...
export const CollectionBasicDetails: React.FC<{
  ...
  const standardTrouserSizes = ["26", "28", "30", "32", "34", "36"];
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {releaseType === "footwear" ? (
              <div className="space-y-3">
                {/* AVAILABLE SHOE SIZES (UK/IN) */}
                <div className="space-y-1.5">
                  <label className="text-[9px] text-amber-500 font-mono uppercase tracking-wider block font-semibold">
                    AVAILABLE SHOE SIZES (UK/IN)
                  </label>
                  <div className="flex flex-wrap gap-1.5">
                    {["6", "7", "8", "9", "10", "11", "12"].map((sz) => {
                      const isSelected = (details.sizes || []).some(
                      ...
                      return (
                        <button
                          ...
                          key={`shoe-${sz}`}
                          onClick={() => {
                            const hasSz = (details.sizes || []).some(
                              (s) => s === sz || s === `UK ${sz}` || s.replace(/^UK\s*/i, "") === sz
                            );
                            const nextSizes = hasSz
                              ? (details.sizes || []).filter(
                                  (s) => s !== sz && s !== `UK ${sz}` && s.replace(/^UK\s*/i, "") !== sz
                                )
                              : [...(details.sizes || []).filter(
                                  (s) => s !== sz && s !== `UK ${sz}` && s.replace(/^UK\s*/i, "") !== sz
