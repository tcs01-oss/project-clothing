...
// Submit a single review or Bulk Import JSON
export async function submitReviews(reviewsData: any | any[], productId: string) {
...
export async function addReviewToFirestore(reviewData: any): Promise<string> {
  try {
    const pId = reviewData.productId || "";
    const result = await submitReviews(reviewData, pId);
...
export async function updateReviewInFirestore(reviewId: string, updateData: any): Promise<void> {
  try {
