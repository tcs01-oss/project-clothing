...
// Helper function to safely read reviews from disk
function readReviewsFromDisk() {
  try {
    if (!fs.existsSync(REVIEWS_PATH)) {
      fs.writeFileSync(REVIEWS_PATH, '[]', 'utf-8');
    }
    const rawData = fs.readFileSync(REVIEWS_PATH, 'utf-8');
...
// Helper function to safely write reviews to disk
function writeReviewsToDisk(reviews: any) {
  fs.writeFileSync(REVIEWS_PATH, JSON.stringify(reviews, null, 2), 'utf-8');
}

// PUBLIC API: Fetch reviews for a specific product
app.get('/api/reviews/:productId', async (req, res) => {
  const { productId } = req.params;
  const allReviews = readReviewsFromDisk();
  
  // Only return reviews that match the requested product
  const productReviews = allReviews.filter((review: any) => review.productId === productId);
  res.json(productReviews);
});

// ADMIN API: Bulk import or add new reviews
app.post('/api/admin/reviews', async (req, res) => {
  try {
    // We accept either a single review object or an array of reviews (for bulk import)
    ...
    const currentReviews = readReviewsFromDisk();
    
    // Append the new reviews to the existing array
    const updatedReviews = [...currentReviews, ...incomingReviews];
    
    writeReviewsToDisk(updatedReviews);
    console.log(`[Auto-Sync] ${incomingReviews.length} reviews added to reviews.json successfully.`);
    
    return res.json({ success: true, message: "Reviews imported successfully!" });
  } catch (error) {
    console.error("Error saving reviews:", error);
    return res.status(500).json({ error: "Failed to save review changes." });
  }
});
...
async function syncToFirestore(): Promise<{ attempted: boolean; failedCollections: string[] }> {
...
// reporting success.
async function saveDb(): Promise<{ firestoreOk: boolean; failedCollections: string[] }> {
  try {
    const data = {
      users,
      userPasswords,
...
app.post("/api/admin/reviews", protect, admin, async (req, res) => {
  const { productId, userName, userEmail, rating, comment, date, status } = req.body;
  ...
  await saveDb();
  res.status(201).json({ success: true, message: "Review created successfully.", review: newReview, product: prod });
});
